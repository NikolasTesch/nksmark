...
> **Status:** implementado · **Reconciliado:** 2026-09-13 · **evidência:** `src/app/api/downloads/zip/route.ts` + `route.test.ts`, `src/components/artwork/DownloadModal.tsx` (botão "Baixar todos (.zip)" :160), `src/app/loja/[slug]/page.tsx` (`handleZipDownloadRequest` :216, `onZipDownloadRequest` :504).
