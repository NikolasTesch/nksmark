...
- [x] A tabela `Order` e os novos campos em `Artwork` e `Role` estão refletidos no Neon de produção/staging. _(evidência: `prisma/schema.prisma` — `enum OrderStatus` :109, `model Order` :117, `Role.CLIENT` :136, `Artwork.priceCents` :148; o `db push` ao Neon é passo de ops documentado em §2.1)_
- [x] O arquivo `src/app/api/admin/sales/route.test.ts` existe e valida as agregações de receita, ticket médio, clientes únicos e variação mensal para ADMIN, retornando `403` para outros perfis. _(evidência: `src/app/api/admin/sales/route.test.ts`)_
- [x] O arquivo `src/app/api/orders/[id]/route.test.ts` existe e valida o polling, garantindo proteção de visibilidade (`404`) e sucesso para dono e `ADMIN` (`200`). _(evidência: `src/app/api/orders/[id]/route.test.ts`)_
- [x] Os testes de componente para `/loja/[slug]/page.tsx` com environment `jsdom` validam os 4 estados do botão de ação. _(evidência: `src/app/loja/[slug]/page.test.tsx` existe e passa; copy atual do botão CLIENT sem compra é "Adicionar ao carrinho — R$ …", assertion alinhada em 2026-09-13)_
- [x] Os testes de componente para `/cadastro/page.tsx` com environment `jsdom` validam loading, sucesso e erro. _(evidência: `src/app/cadastro/page.test.tsx`)_
- [x] Todos os testes da aplicação (`npm test`) estão passando sem erros. _(evidência: suíte completa verde 2x consecutivas em 2026-09-13 — 142/142 testes, 24 arquivos, sem serviços externos; `pool: 'threads'` no vitest.config.ts resolveu flake de worker no Windows; passo `Run Unit Tests` adicionado ao CI)_
- [x] O webhook do Mercado Pago valida a assinatura (`MP_WEBHOOK_SECRET`) e responde `200`. _(evidência: `src/app/api/payments/webhook/route.ts` :45 `verifyWebhookSignature`)_
- [x] Os documentos `spec.md` e `PRD.md` estão atualizados para o marketplace. _(evidência: `PRD.md` Fase 2 itens de pagamento marcados; `spec.md` corrigido nesta revisão)_
